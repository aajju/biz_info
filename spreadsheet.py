...
def save_data_project(items, i):
    ...
    if i == 0:  # 발주예정 용역
        ...
        for item in items:
            ...
            worksheet.append_row(
                [
                    datetime.strptime(item["creatPnttm"], "%Y-%m-%d %H:%M:%S")
                    .date()
                    .strftime("%Y-%m-%d"),  # 등록일시
                    item["reqstBeginEndDe"],  # 신청기간
                    item["pblancNm"],  # 공고명
                    item["pldirSportRealmLclasCodeNm"],  # 대분류
                    item["pldirSportRealmMlsfcCodeNm"],  # 대분류2
                    # item["totCnt"], # 전체건수
                    # item["inqireCo"], # 조회수
                    item["jrsdInsttNm"],  # 소관부처
                    item["excInsttNm"],  # 수행기관
                    item["refrncNm"],  # 담당부서
                    item["printFileNm"],  # 파일명
                    item["printFlpthNm"],  # 파일다운로드
                    item["trgetNm"],  # 지원대상  - 중소기업, 소상공인
                    # item["bsnsSumryCn"],  # 개요내용
                    # item["reqstMthPapersCn"],  # 접수방법
                    "https://www.bizinfo.go.kr"
                    + item["pblancUrl"],  # 공고url - "https://www.bizinfo.go.kr"  붙여야함
                    item["rceptEngnHmpgUrl"],  # 접수url
                    # item["hashtags"], # 해시태그
                    # item["pblancId"], # 공고번호
                ]
            )
